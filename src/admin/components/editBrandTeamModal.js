...
const EditBrandTeamModal = ({currentRecord, visible,  handleCancel, handleFinish}) => {
   ...
   const addNewMember = async () => {
    if(teamInput){
      ...
      const endPoint = API.ADD_PARTNER_TEAMMEMBER;
      try {
   ...
   const removeTeam = async (userId) => {
    ...
    const endPoint = API.REMOVE_BRAND_TEAMMEMBER;
    try {
      const res = await axios.post(endPoint, requestBody);
      if(res.status === 200){
...
export default EditBrandTeamModal;